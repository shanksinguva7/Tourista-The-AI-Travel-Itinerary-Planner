...
export const getUsersAndTripsStats = async (): Promise<DashboardStats> => {
    ...
    return {
        ...
        userRole: {
            ...
            currentMonth: filterByDate(filterUsersByRole("user"), "joinedAt", startCurrent),
...
export const getUserGrowthPerDay = async () => {
    ...
    return Object.entries(userGrowth).map(([day, count]) => ({ count: Number(count), day }));
};
...
export const getTripsCreatedPerDay = async () => {
    ...
    return Object.entries(tripsGrowth).map(([day, count]) => ({ count: Number(count), day }));
};
